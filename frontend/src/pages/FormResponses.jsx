...
import { ArrowLeft, Download, Filter, Calendar, Search, BarChart3, PieChart, TrendingUp, Users, Loader2, Trash2, AlertTriangle, MapPin, ShieldAlert, ShieldCheck } from 'lucide-react';
...
const FormResponses = () => {
  ...
  const [spamFilter, setSpamFilter] = useState('all'); // 'all' | 'flagged' | 'clean'
  ...
  useEffect(() => {
    const loadSpamStats = async () => {
      try {
        const res = await formAPI.getSpamStats(formId);
        if (res.success) setSpamStats(res.data);
      } catch (e) {
        console.warn('Spam stats load failed:', e?.message || e);
      }
    };
    if (formId) loadSpamStats();
  }, [formId]);
  ...
  if (spamFilter === 'flagged') {
    filteredResponses = filteredResponses.filter(r => r.flagged);
              ...
              <div className="mt-6 p-4 bg-blue-50 rounded-lg">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex space-x-2">
                    ...
                    */}
                    ...
                    <button
                      onClick={() => setSelectedVisualization('spam')}
                      className={`px-3 py-1 rounded text-sm ${selectedVisualization === 'spam' ? 'bg-red-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-50'}`}
                    >
                ...
                <div className="bg-white rounded-lg p-4">
                  ...
                  */}
                  ...
                  {selectedVisualization === 'spam' && (
                    <div className="space-y-4">
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="bg-gradient-to-r from-blue-500 to-blue-600 text-white p-4 rounded-lg">
                          <p className="text-blue-100 text-sm">Total Responses</p>
                          <p className="text-2xl font-bold">{spamStats?.totalResponses || 0}</p>
                        </div>
                        <div className="bg-gradient-to-r from-red-500 to-red-600 text-white p-4 rounded-lg">
                          <p className="text-red-100 text-sm">Flagged as Spam</p>
                          <p className="text-2xl font-bold">{spamStats?.flaggedCount || 0}</p>
                        </div>
                        <div className="bg-gradient-to-r from-green-500 to-green-600 text-white p-4 rounded-lg">
                          <p className="text-green-100 text-sm">Clean Responses</p>
                          <p className="text-2xl font-bold">{(spamStats?.totalResponses || 0) - (spamStats?.flaggedCount || 0)}</p>
                        </div>
                      </div>

                      {spamStats?.flaggedEmails?.length > 0 && (
                        <div className="bg-white border border-red-200 rounded-lg p-4">
                          <h4 className="font-semibold text-red-800 mb-3 flex items-center gap-2">
                            <ShieldAlert className="w-4 h-4" /> Flagged Submissions
                          </h4>
                          <div className="space-y-2">
                            {spamStats.flaggedEmails.map((item, idx) => (
                              <div key={idx} className="flex items-center justify-between p-3 bg-red-50 rounded-lg border border-red-100">
                                <div>
                                  <span className="font-medium text-gray-900">{item.email}</span>
                                  <div className="flex flex-wrap gap-1 mt-1">
                                    {item.reasons?.map((reason, i) => (
                                      <span key={i} className="text-xs bg-red-100 text-red-700 px-2 py-0.5 rounded-full">{reason}</span>
                                    ))}
                                  </div>
                                </div>
                                <span className="text-sm font-bold text-red-600">{Math.round((item.spamScore || 0) * 100)}%</span>
                              </div>
                            ))}
                          </div>
                        </div>
                      ...
                      {(!spamStats?.flaggedEmails || spamStats.flaggedEmails.length === 0) && (
                        <div className="text-center py-8">
                          <ShieldCheck className="w-12 h-12 text-green-400 mx-auto mb-4" />
                          <p className="text-gray-500">No spam detected! All responses look clean.</p>
                        </div>
                      )}
                    </div>
                  )}
                ...
                <table className="w-full min-w-full">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {filteredResponses.map((response) => (
                      <tr key={response.id} className={`hover:bg-gray-50 ${response.flagged ? 'bg-red-50 hover:bg-red-100' : ''}`}>
                        <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-sm">
                          {response.flagged ? (
                            <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium bg-red-100 text-red-700" title={response.spamReasons?.join(', ') || 'Flagged as spam'}>
